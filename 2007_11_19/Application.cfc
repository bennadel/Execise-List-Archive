...
<cfcomponent 
	...
	<cffunction 
		...
		<cflock
			...
			<!--- 
				Create a singleton of the Service Factory and cache it in 
				the APPLICATION scope. This will allow all the service 
				objects to be cached for the duration of the application.
			--->
			<cfset APPLICATION.ServiceFactory = THIS.CreateCFC( "ServiceFactory" ).Init(
				APPLICATION.DSN
				) />
	...
	<cffunction
		...
		returntype="any"
		output="false"
		hint="Utilitye function for creating a CFCs relative the root path.">
		...
		<cfargument
			...
			type="string"
			required="true"
			hint="The name of the CFC."
			/>
			
		<!--- Return the non-initialized CFC instance. --->
		<cfreturn CreateObject( "component", "cfc.#ARGUMENTS.CFC#" ) />
	</cffunction>
